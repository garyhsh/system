...
extern int brcm_get_btaddrstr(char* bdaddr_str);
void
sw_read_default_bdaddr()
{
	int sz;
	char bdaddr[18];
	int len = 17;
	...
	brcm_get_btaddrstr(bdaddr);
	
	if (strlen(bdaddr) != len) {
		fprintf(stderr, "%s unexpected bt mac size %d", __FUNCTION__, strlen(bdaddr));
		return;
	}
	
	if (debug) {
		printf("SW default bdaddr of %s\n", bdaddr);	
	}
