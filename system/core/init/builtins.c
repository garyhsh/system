...
int logerrortofile(const char *filedir)
{
	pid_t child;
	int status;
	time_t t;
	char filename[FILE_NAME_SIZE] = {0};
	char abspath[FILE_NAME_SIZE] = {0};
	ERROR("logerrortofile");
	if(strlen(filedir)>FILE_NAME_SIZE)
	{
		ERROR("File Dir too long!");
		return 0;
	}
	strcpy(abspath,filedir);
	ERROR("afspath is %s",abspath);
	if(filedir[strlen(filedir)-1]!='/')
		strcat(abspath,"/");
	time(&t);
	strftime(filename,sizeof(filename),"%Y-%m-%d-%I-%M-%S.txt",localtime(&t));
	strcat(abspath,filename);
	ERROR("File ABS Path is %s",abspath);
	child = fork();		
    if (child == 0) {
        	execl("/system/bin/logcat","/system/bin/logcat","-f",abspath, NULL);
        	exit(-1);
   	}
	sleep(6);
	sync();
...
int do_datarestore(int nargs,char **args)
{
	ERROR("do_datarestore %s",args[1]);
	char *str5 = "/data/system";
	char *str6 = "/cache/dsbackup/datasystem.tar";
	char *str7 = "/cache/dsbackup/datasystem_back.tar";
	char *code1[]={"/system/bin/busybox","/system/bin/busybox","tar","-cvf","/cache/dsbackup/datasystem.tar","/data/system",NULL};
	char *code2[]={"/system/bin/busybox","/system/bin/busybox","tar","-xvf","/cache/dsbackup/datasystem.tar","-C","/",NULL};

	if((args[1][0] == 'b')||(args[1][0] == 'B'))
	{
		ERROR("b");
		if(exist(str6))
		    rename(str6,str7);
	...
	}else if((args[1][0] == 'r')||(args[1][0]=='R'))
	{
	  ...
	  {
	  	unlink_recursive(str5);
	  }
	}else if((args[1][0] == 'l')||(args[1][0]=='L'))
	{
		ERROR("l");
		logerrortofile("/cache/dsbackup/logfile");
	}else
	{
...
int do_dispe2fsck(int nargs,char **args)
{
	...
	if(nargs<2){
		ERROR("usage:dispe2fsck /dev/block/nandx");
		return 0;
	}
